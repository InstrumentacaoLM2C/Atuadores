...
int sensorProximidadeCima = PA1;  //pin do sensor de proximidade da direita
int sensorProximidadeBaixo = PA0; //pin do sensor de proximidade da esquerda
...
// Direção - = Digital PB7 (CCW), Pulsos - = Digital PB6 (CLK)
AccelStepper stepper1(1, PB6, PB7);
int direcao1 = 1; // Direção = 1 (CCW) Direção = -1 (CW)
int driverEn1 = PB8; //en- pin
...
// Direção - = Digital PB3 (CCW), Pulsos - = Digital PB4 (CLK)
AccelStepper stepper2(1, PB4, PB3);
int direcao2 = 1; // Direção = 1 (CCW) Direção = -1 (CW)
int driverEn2 = PB5; //en- pin 
...
void sensor_Proximidade(){
  int sensorC = analogRead(sensorProximidadeCima);
  int sensorB = analogRead(sensorProximidadeBaixo);

  if(sensorC >= 800){sensorC = 1;}
  else{sensorC =0;}

  if(sensorB >= 800){sensorB = 1;}
  else{sensorB = 0;}



  if(((sensorC == 1) || (sensorB == 1)) && motorParou1 == 0){
    paraMotor1();
    motorParou1 = 1;
    parar_calibracao = 1;
    Serial.println("/O motor foi travado pelo sensor indutivo!"); //"Fim de Curso" colocar isso
...
void subsidencia() { // Função que movimenta o motor para frente e para trás (2 voltas completas) // matar isso
